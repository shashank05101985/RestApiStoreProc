...
CREATE PROC INSERT_UPDATE_CITY
@name varchar(155),
@code varchar(15),
@parentid int,
@createby int,
...
IF  @id = 0
BEGIN
insert into city(name,code,parent_id,create_by,update_by) values(@name,@code,@parentid,@createby,@updateby);
SELECT SCOPE_IDENTITY();
END;
ELSE
BEGIN
update city set name=@name,code=@code,parent_id=@parentid,update_by=@updateby,update_time=GETDATE() where id =@id;
END;